...
SELECT P.ProductID, PM.Name AS ProductName, P.Name AS ProductModel,PM.Summary
...
-- Then use the table variable to filter a query that returns the product ID, name, and color from the SalesLT.Product 
-- table so that only products with a color listed in the table variable are returned. You'll need to use NVARCHAR(15) 
...
-- Write a query that uses this function to return a list of all products including their parent category 
-- and their own category. Make sure to use the aliases provided, and default column names elsewhere.
...
-- each customer. Use a derived table or a common table expression to retrieve the details for each sales 
-- order, and then query the derived table or CTE to aggregate and group the data. Make sure to use the aliases 
