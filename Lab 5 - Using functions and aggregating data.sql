...
-- formatted as upper case and a column named ApproxWeight with the weight of each product rounded 
-- to the nearest whole unit. Make sure to use the aliases provided, and default column names 
-- elsewhere.
...
-- and month in which AdventureWorks started selling each product. The month should be displayed as 
-- the month name (e.g. 'January'). Make sure to use the aliases provided, and default column names 
-- elsewhere.
SELECT ProductID, UPPER(Name) AS ProductName, ROUND(Weight, 0) AS ApproxWeight,
...
-- Extend your query to filter the product returned so that only products with a numeric size are 
-- included. Make sure to use the aliases provided, and default column names elsewhere.
SELECT ProductID, UPPER(Name) AS ProductName, ROUND(Weight, 0) AS ApproxWeight,
       YEAR(SellStartDate) as SellStartYear,
       DATENAME(m, SellStartDate) as SellStartMonth,
       LEFT(ProductNumber, 2) AS ProductType
...
-- Write a query that returns a list of company names with a ranking of their place in a list of 
-- highest TotalDue values from the SalesOrderHeader table. Make sure to use the aliases provided, 
...
-- sum of the LineTotal from the SalesLT.SalesOrderDetail table, with the results sorted in 
-- descending order of total revenue. Make sure to use the aliases provided, and default column 
-- names elsewhere.
...
-- than 1000. Make sure to use the aliases provided, and default column names elsewhere.
SELECT Name, SUM(LineTotal) AS TotalRevenue
FROM SalesLT.SalesOrderDetail AS SOD
JOIN SalesLT.Product AS P
ON SOD.ProductID = P.ProductID
